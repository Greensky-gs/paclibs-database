...
static int is_nested(char *value)
{
...
static int is_commented(char *line)
{
    if ('#' == line[0]) {
        return 1;
    }
...
static void set_variable(char *name, char *original, int overwrite)
{
    char *parsed;
...
static void parse(FILE *file, int overwrite)
{
